...
import (
	...
	folderCore "github.com/amitshekhariitbhu/go-backend-clean-architecture/domain/music/file_entity/folder/core"
...
func NewFolderRepo(db mongo.Database, collection string) folderCore.FolderRepository {
	return &folderRepo{
		db:         db,
...
func (r *folderRepo) ListFolders(path string) ([]folderCore.FolderEntry, error) {
	// 处理根目录情况
	if path == "" {
	...
	var folders []folderCore.FolderEntry
	for _, entry := range entries {
		if entry.IsDir() {
			fullPath := filepath.Join(path, entry.Name())
			folders = append(folders, folderCore.FolderEntry{
				Name: entry.Name(),
				Path: fullPath,
			})
		}
	...
	if path != "/" && runtime.GOOS != "windows" {
		...
		folders = append([]folderCore.FolderEntry{
...
// 获取根目录（跨平台实现）
func (r *folderRepo) getRootDirectories() ([]folderCore.FolderEntry, error) {
	if runtime.GOOS == "windows" {
		return r.getWindowsDrives()
	}
	return []folderCore.FolderEntry{{Name: "/", Path: "/", IsRoot: true}}, nil
}

// Windows平台：获取所有驱动器
func (r *folderRepo) getWindowsDrives() ([]folderCore.FolderEntry, error) {
	var drives []folderCore.FolderEntry
	for drive := 'A'; drive <= 'Z'; drive++ {
		drivePath := string(drive) + ":\\"
		if _, err := os.Stat(drivePath); err == nil {
			drives = append(drives, folderCore.FolderEntry{
				Name:   drivePath,
				Path:   drivePath,
				IsRoot: true,
			})
...
func (r *folderRepo) Insert(ctx context.Context, folder *folderCore.LibraryFolderMetadata) error {
...
func (r *folderRepo) FindLibrary(
	...
	folderPath string,
	folderType int,
) (*folderCore.LibraryFolderMetadata, error) {
	collection := r.db.Collection(r.collection)
	...
	var folder folderCore.LibraryFolderMetadata
...
func (r *folderRepo) GetAllByType(ctx context.Context, folderType int) ([]*folderCore.LibraryFolderMetadata, error) {
	collection := r.db.Collection(r.collection)
	...
	var allFolders []*folderCore.LibraryFolderMetadata
	for cursor.Next(ctx) {
		var folder folderCore.LibraryFolderMetadata
	...
	// 检查路径存在性并过滤
	validFolders := make([]*folderCore.LibraryFolderMetadata, 0, len(allFolders))
...
func (r *folderRepo) UpdateStats(
	...
	fileCount int, status folderCore.LibraryStatus,
) error {
	collection := r.db.Collection(r.collection)
...
func (r *folderRepo) DetectingDuplicates(ctx context.Context, folderPath string, folderType int) (*folderCore.LibraryFolderMetadata, error) {
	...
	var existingLib folderCore.LibraryFolderMetadata
...
func (r *folderRepo) Create(ctx context.Context, folder *folderCore.LibraryFolderMetadata) error {
	// 检查重复媒体库
	if existing, _ := r.DetectingDuplicates(ctx, folder.FolderPath, folder.FolderType); existing != nil {
		return folderCore.ErrLibraryDuplicate
	}

	folder.ID = primitive.NewObjectID()
	folder.CreatedAt = time.Now()
	folder.UpdatedAt = time.Now()
	folder.Status = folderCore.StatusActive
...
func (r *folderRepo) DeleteByID(ctx context.Context, id primitive.ObjectID) error {
	// 检查媒体库状态
	if lib, err := r.GetByID(ctx, id); err == nil && lib.Status == folderCore.StatusScanning {
		return folderCore.ErrLibraryInUse
	}
	...
	if result == 0 {
		return folderCore.ErrLibraryNotFound
	}
	return err
}
...
func (r *folderRepo) IsLibraryInUse(ctx context.Context, id primitive.ObjectID) (bool, error) {
	// 查询当前媒体库状态
	var lib folderCore.LibraryFolderMetadata
	filter := bson.M{"_id": id}
	...
	if err != nil {
		if errors.Is(err, driver.ErrNoDocuments) {
			return false, folderCore.ErrLibraryNotFound
		}
		return false, fmt.Errorf("数据库查询失败: %w", err)
	}

	// 核心逻辑：状态检测
	return lib.Status == folderCore.StatusScanning || lib.Status == folderCore.StatusDisabled, nil
}
...
func (r *folderRepo) GetByID(
	...
	switch {
	case errors.Is(err, driver.ErrNoDocuments):
		return nil, folderCore.ErrLibraryNotFound // 明确返回领域错误[1](@ref)
	case err != nil:
		return nil, fmt.Errorf("数据库操作失败: %w", err)
	default:
		return &lib, nil
...
func (r *folderRepo) GetAll(ctx context.Context) ([]*folderCore.LibraryFolderMetadata, error) {
	...
	var libraries []*folderCore.LibraryFolderMetadata
...
func (r *folderRepo) UpdateStatus(ctx context.Context, id primitive.ObjectID, status folderCore.LibraryStatus) error {
