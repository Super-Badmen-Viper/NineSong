...
func NewFileRepo(db mongo.Database, collection string) fileCore.FileRepository {
	return &fileRepo{db: db, collection: collection}
}

func (r *fileRepo) FindByPath(ctx context.Context, path string) (*fileCore.FileMetadata, error) {
	...
	// 执行查询
	var result fileCore.FileMetadata
...
func (r *fileRepo) Upsert(ctx context.Context, file *fileCore.FileMetadata) error {
