...
import (
	...
	recommendCore "github.com/amitshekhariitbhu/go-backend-clean-architecture/domain/music/recommend/recommend/core"
...
type DataFetcher interface {
	GetAnnotations(ctx context.Context, itemType string, algorithmType string) ([]annotationCore.AnnotationMetadata, error)
	GetItems(ctx context.Context, targetCollection string, annotations []annotationCore.AnnotationMetadata) ([]bson.M, error)
}
...
type recommendRepository struct {
	...
	// 缓存相关字段
	wordCloudCache map[string][]wordCloudCore.WordCloudMetadata
...
func NewRecommendRepository(db mongo.Database, collection string) recommendCore.RecommendRepository {
...
func (r *recommendRepository) GetGeneralRecommendations(
	...
	case "artist":
		itemType = "artist"
		targetCollection = shared.CollectionFileEntityAudioSceneArtist
	case "album":
		itemType = "album"
		targetCollection = shared.CollectionFileEntityAudioSceneAlbum
	case "media":
		itemType = "media"
		targetCollection = shared.CollectionFileEntityAudioSceneMediaFile
	case "media_cue":
		itemType = "media_cue"
		targetCollection = shared.CollectionFileEntityAudioSceneMediaFileCue
	default:
		itemType = "media"
		targetCollection = shared.CollectionFileEntityAudioSceneMediaFile
	}
...
func (r *recommendRepository) getRecommendationsFromAnnotations(
	...
	annotations []annotationCore.AnnotationMetadata,
	recommendType string,
	limit int,
	...
	for cursor.Next(ctx) {
		...
		result, err := r.createRecommendationResult(
			...
			annotations,
			[]wordCloudCore.WordCloudMetadata{},
			[]wordCloudCore.WordCloudRecommendation{},
		)
		if err != nil {
...
func (r *recommendRepository) getUnifiedRecommendationWorkflow(
	...
	// 获取词云集合
	wordCloudColl := r.db.Collection(shared.CollectionFileEntityAudioSceneMediaFileWordCloud)
	...
	// 初始化词云标签数组
	var wordCloudTags []wordCloudCore.WordCloudMetadata
	...
	if err != nil {
		...
		// 在缓存中查找匹配的标签
		var matchedTags []wordCloudCore.WordCloudMetadata
	...
	// 获取集合引用
	annotationColl := r.db.Collection(shared.CollectionFileEntityAudioSceneAnnotation)
	itemColl := r.db.Collection(targetCollection)
...
func (r *recommendRepository) getAnnotationBasisFromAnnotations(
	annotations []annotationCore.AnnotationMetadata,
	limit int,
) []annotationCore.AnnotationMetadata {
	if len(annotations) == 0 {
		return []annotationCore.AnnotationMetadata{}
	}
...
func (r *recommendRepository) getTagBasisFromWordCloud(
	wordCloudTags []wordCloudCore.WordCloudMetadata,
	limit int,
) []wordCloudCore.WordCloudMetadata {
	if len(wordCloudTags) == 0 {
		return []wordCloudCore.WordCloudMetadata{}
	}
...
func (r *recommendRepository) getRelatedItemsFromWordCloud(
	...
	var relatedItems []wordCloudCore.WordCloudRecommendation

	for i, tag := range wordCloudTags {
		if i >= limit {
			break
		}
		relatedItems = append(relatedItems, wordCloudCore.WordCloudRecommendation{
			ID:    tag.ID,
			Type:  tag.Type,
			Name:  tag.Name,
...
func (r *recommendRepository) createRecommendationResult(
	...
	basis []string,
	annotationBasis []annotationCore.AnnotationMetadata,
	tagBasis []wordCloudCore.WordCloudMetadata,
	relatedItems []wordCloudCore.WordCloudRecommendation,
) (interface{}, error) {
	// 根据推荐类型创建具体的推荐结果
	switch recommendType {
	case "artist":
...
func (r *recommendRepository) getCachedWordCloudTags(ctx context.Context) ([]wordCloudCore.WordCloudMetadata, error) {
	// 检查缓存是否有效
	r.cacheMutex.RLock()
	...
	var allWordCloudTags []wordCloudCore.WordCloudMetadata
	...
	if r.wordCloudCache == nil {
		r.wordCloudCache = make(map[string][]wordCloudCore.WordCloudMetadata)
	}
...
func (r *recommendRepository) getCachedWordCloudTagsByType(ctx context.Context, wordCloudType string) ([]wordCloudCore.WordCloudMetadata, error) {
	// 检查缓存是否有效
...
func (r *recommendRepository) GetAnnotations(
	...
	itemType string,
	algorithmType string,
) ([]annotationCore.AnnotationMetadata, error) {
	annotationColl := r.db.Collection(shared.CollectionFileEntityAudioSceneAnnotation)

	// 记录开始处理的日志 - 标记为第一阶段开始
	r.logInfo("【推荐系统-第一阶段】开始获取用户行为数据，itemType=%s, algorithmType=%s", itemType, algorithmType)
	r.logDebug("[第一阶段-步骤1] 准备从集合%s查询注释数据", shared.CollectionFileEntityAudioSceneAnnotation)
...
func (r *recommendRepository) GetItems(
	...
	// 收集item_id用于后续查询
	var itemIds []string
	itemIdToAnnotation := make(map[string]annotationCore.AnnotationMetadata)
	for _, annotation := range annotations {
