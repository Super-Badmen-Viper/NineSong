...
func NewWordCloudRepository(db mongo.Database, collection string) wordCloudCore.WordCloudRepository {
...
// BulkUpsert 批量创建/更新词云元数据
func (w *wordCloudRepository) BulkUpsert(ctx context.Context, files []*wordCloudCore.WordCloudMetadata) (int, error) {
	coll := w.db.Collection(w.collection)
	bulk := coll.BulkWrite()
...
// GetAll 获取所有词云元数据
func (w *wordCloudRepository) GetAll(ctx context.Context) ([]*wordCloudCore.WordCloudMetadata, error) {
	coll := w.db.Collection(w.collection)
	...
	var results []*wordCloudCore.WordCloudMetadata
	for cursor.Next(ctx) {
		var item wordCloudCore.WordCloudMetadata
