...
func NewMediaFileCueRepository(db mongo.Database, collection string) mediaFileCueCore.MediaFileCueRepository {
...
// UpsertWithResult 创建或更新CUE文件元数据（返回结果版本）
func (r *mediaFileCueRepository) UpsertWithResult(ctx context.Context, file *mediaFileCueCore.MediaFileCueMetadata) (*mediaFileCueCore.MediaFileCueMetadata, error) {
...
func (r *mediaFileCueRepository) GetByPath(ctx context.Context, path string) (*mediaFileCueCore.MediaFileCueMetadata, error) {
	coll := r.db.Collection(r.collection)
...
func (r *mediaFileCueRepository) DeleteAllInvalid(
	...
	cur, err := coll.Find(ctx, bson.M{}, options.Find().SetProjection(bson.M{"_id": 1, "path": 1, "performer_id": 1}))
	if err != nil {
	...
	for cur.Next(ctx) {
		...
		if !isValid {
			...
			if doc.PerformerID != "" {
				performerObjID, err := primitive.ObjectIDFromHex(doc.PerformerID)
				if err == nil {
					artistToIDs[performerObjID] = append(artistToIDs[performerObjID], doc.ID)
				}
			}
		}
	}
...
func (r *mediaFileCueRepository) DeleteAll(ctx context.Context) (int64, error) {
	coll := r.db.Collection(r.collection)
	return coll.DeleteMany(ctx, bson.M{})
}

// GetByFolder 根据文件夹获取CUE文件路径列表
...
// MediaCueCountByArtist 统计艺术家的CUE文件数量
...
// GuestMediaCueCountByArtist 统计艺术家的嘉宾CUE文件数量
...
// MediaCountByAlbum 统计专辑的CUE文件数量
...
func (r *mediaFileCueRepository) inspectMediaCue(
	...
	if len(invalidPaths) > 0 {
		...
		sampleSize := 3
		if len(invalidPaths) < sampleSize {
			sampleSize = len(invalidPaths)
		}
		log.Printf("检测到 %d 个无效CUE媒体项，示例: %v...",
			len(invalidPaths),
			invalidPaths[:sampleSize])
	} else {
		return 0, nil // 没有无效项
	}
...
// InspectMediaCueCountByArtist 检查艺术家的CUE文件计数
...
// InspectGuestMediaCueCountByArtist 检查艺术家的嘉宾CUE文件计数
