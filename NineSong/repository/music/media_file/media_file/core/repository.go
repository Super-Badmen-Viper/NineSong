...
import (
	...
	"github.com/amitshekhariitbhu/go-backend-clean-architecture/domain/shared/util"
...
func NewMediaFileRepository(db mongo.Database, collection string) mediaFileCore.MediaFileRepository {
	if runtime.GOOS == "windows" || runtime.GOOS == "darwin" {
		jieba := gojieba.NewJieba()
		stopWords := util.LoadCombinedStopWords()
...
func (r *mediaFileRepository) GetAllGenre(ctx context.Context) ([]wordCloudCore.WordCloudMetadata, error) {
	...
	results := make([]wordCloudCore.WordCloudMetadata, len(rawResults))
	for i, item := range rawResults {
		results[i] = wordCloudCore.WordCloudMetadata{
...
func (r *mediaFileRepository) GetHighFrequencyWords(
	ctx context.Context,
	limit int,
) ([]wordCloudCore.WordCloudMetadata, error) {
	...
	for word, count := range globalWordMap {
		if h.Len() < limit {
			heap.Push(h, util.WordCount{Word: word, Count: count})
	...
	wordCounts := make([]mediaFileCore.WordCloudMetadata, h.Len())
	tempList := make([]util.WordCount, h.Len())
	for i := 0; h.Len() > 0; i++ {
		tempList[i] = heap.Pop(h).(util.WordCount)
	}
	...
	for i, wc := range tempList {
		wordCounts[i] = wordCloudCore.WordCloudMetadata{
			ID:    primitive.NewObjectID(),
			Name:  wc.Word,
			Count: wc.Count,
			Type:  "media_file",
...
func (r *mediaFileRepository) GetRecommendedByKeywords(
	...
	keywords []string,
	limit int,
) ([]wordCloudCore.WordCloudRecommendation, error) {
	coll := r.db.Collection(r.collection)
	if len(keywords) == 0 {
		return []wordCloudCore.WordCloudRecommendation{}, nil
	}
	...
	// 6. 转换为响应模型
	results := make([]wordCloudCore.WordCloudRecommendation, len(files))
	for i, f := range files {
		results[i] = wordCloudCore.WordCloudRecommendation{
			ID:    f.ID,
			Type:  "media_file",
			Name:  f.Title,
...
func (r *mediaFileRepository) GetAllCounts(ctx context.Context) ([]mediaFileCore.MediaFileCounts, error) {
	coll := r.db.Collection(r.collection)
	...
	var results []mediaFileCore.MediaFileCounts
...
func (r *mediaFileRepository) UpsertWithResult(ctx context.Context, file *mediaFileCore.MediaFileMetadata) (*mediaFileCore.MediaFileMetadata, error) {
	coll := r.db.Collection(r.collection)
	now := time.Now().UTC()
...
func (r *mediaFileRepository) BulkUpsert(ctx context.Context, files []*mediaFileCore.MediaFileMetadata) (int, error) {
	coll := r.db.Collection(r.collection)
...
func (r *mediaFileRepository) GetAll(ctx context.Context) ([]*mediaFileCore.MediaFileMetadata, error) {
	coll := r.db.Collection(r.collection)
	...
	var files []*mediaFileCore.MediaFileMetadata
	if err := cursor.All(ctx, &files); err != nil {
...
func (r *mediaFileRepository) GetByID(ctx context.Context, id primitive.ObjectID) (*mediaFileCore.MediaFileMetadata, error) {
	coll := r.db.Collection(r.collection)
	result := coll.FindOne(ctx, bson.M{"_id": id})

	var file mediaFileCore.MediaFileMetadata
...
func (r *mediaFileRepository) GetByPath(ctx context.Context, path string) (*mediaFileCore.MediaFileMetadata, error) {
	coll := r.db.Collection(r.collection)
...
func (r *mediaFileRepository) GetFilesWithMissingMetadata(ctx context.Context, folderPath string, folderType int) ([]*mediaFileCore.MediaFileMetadata, error) {
	coll := r.db.Collection(r.collection)
	...
	if len(fileModTimeMap) == 0 {
		return []*mediaFileCore.MediaFileMetadata{}, nil
	}
	...
	// 4. 比对时间，找出真正需要更新的文件
	var filesToUpdate []*mediaFileCore.MediaFileMetadata
	...
	for path, fileModTime := range fileModTimeMap {
		if dbModTime, exists := dbModTimeMap[path]; !exists || fileModTime.After(dbModTime.Add(timeError)) {
			...
			filesToUpdate = append(filesToUpdate, &mediaFileCore.MediaFileMetadata{
				Path: path,
			})
		}
	}
