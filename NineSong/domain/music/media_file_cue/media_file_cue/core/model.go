...
type MediaFileCueMetadata struct {
	...
	AllArtistIDs []artistCore.ArtistIDPair `bson:"all_artist_ids"` // 所有参与艺术家的唯一标识符列表
}

// CueConfig CUE文件相关资源信息
...
// MediaFileCueFilterCounts CUE文件过滤计数
...
type CueTrack struct {
	...
	Extended            mediaFileCore.MediaFileMetadata `bson:"cue_track_extended"` // 嵌入 MediaFileMetadata 以复用通用字段
}

// Validate 验证CUE文件元数据
func (m *MediaFileCueMetadata) Validate() error {
	// 可以添加验证逻辑
	return nil
}

// SetTimestamps 设置时间戳
func (m *MediaFileCueMetadata) SetTimestamps() {
	now := time.Now()
...
// ToUpdateDoc 转换为更新文档（用于Upsert操作）
