...
func LoadCombinedStopWords() map[string]bool {
	...
	// 直接将4个停用词文件的内容解析并合并
	parseToStopwords(combined, baiduStopWordsContent)
	parseToStopwords(combined, cnStopWordsContent)
	parseToStopwords(combined, hitStopWordsContent)
	parseToStopwords(combined, scuStopWordsContent)

	log.Printf("已加载停用词总数: %d", len(combined))
	return combined
}

// 解析停用词内容并添加到集合中
func parseToStopwords(dest map[string]bool, content string) {
