...
type MediaLibrarySyncRecordUseCase struct {
	mediaLibrarySyncRecordRepo domainCore.MediaLibrarySyncRecordRepository
...
func (uc *MediaLibrarySyncRecordUseCase) CreateSyncRecord(ctx context.Context, syncRecord *domainCore.MediaLibrarySyncRecord) (*domainCore.MediaLibrarySyncRecordResponse, error) {
	...
	// 转换为路由模型返回
	routeModel := domainCore.ConvertDBToRouteModel(syncRecord)
	return routeModel, nil
}

// UpdateSyncRecord 更新媒体库同步记录
func (uc *MediaLibrarySyncRecordUseCase) UpdateSyncRecord(ctx context.Context, id string, syncRecord *domainCore.MediaLibrarySyncRecord) (*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
	...
	update := bson.M{"$set": syncRecord}
	_, err = uc.mediaLibrarySyncRecordRepo.UpdateByID(ctx, objectID, update)
	if err != nil {
		return nil, err
	}

	// 获取更新后的记录
	updatedRecord, err := uc.mediaLibrarySyncRecordRepo.GetByID(ctx, objectID)
	if err != nil {
		return nil, err
	}

	// 转换为路由模型返回
	routeModel := domainCore.ConvertDBToRouteModel(updatedRecord)
	return routeModel, nil
}

// GetSyncRecordByID 根据ID获取同步记录
func (uc *MediaLibrarySyncRecordUseCase) GetSyncRecordByID(ctx context.Context, id primitive.ObjectID) (*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
	...
	// 转换为路由模型返回
	routeModel := domainCore.ConvertDBToRouteModel(result)
	return routeModel, nil
}

// GetSyncRecordByMediaFileID 根据媒体文件ID获取同步记录
func (uc *MediaLibrarySyncRecordUseCase) GetSyncRecordByMediaFileID(ctx context.Context, mediaFileID primitive.ObjectID) (*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
...
func (uc *MediaLibrarySyncRecordUseCase) GetSyncRecordByMediaLibraryAudioID(ctx context.Context, mediaLibraryAudioID primitive.ObjectID) (*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
...
func (uc *MediaLibrarySyncRecordUseCase) GetSyncRecordsByStatus(ctx context.Context, status string) ([]*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
	...
	// 转换为路由模型返回
	var routeResults []*domainCore.MediaLibrarySyncRecordResponse
	for _, dbResult := range dbResults {
		routeModel := domainCore.ConvertDBToRouteModel(dbResult)
...
func (uc *MediaLibrarySyncRecordUseCase) GetSyncRecordsByType(ctx context.Context, syncType string) ([]*domainCore.MediaLibrarySyncRecordResponse, error) {
	ctx, cancel := context.WithTimeout(ctx, uc.timeout)
	defer cancel()
