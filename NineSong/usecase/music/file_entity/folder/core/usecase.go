...
type LibraryUsecase interface {
	BrowseFolders(ctx context.Context, path string) ([]folderCore.FolderEntry, error)

	CreateLibrary(ctx context.Context, name, path string, folderType int) (*folderCore.LibraryFolderMetadata, error)
	DeleteLibrary(ctx context.Context, id string) error
	UpdateLibrary(ctx context.Context,
		id primitive.ObjectID,
		newName string,
		newFolderPath string) error
	GetLibraries(ctx context.Context) ([]*folderCore.LibraryFolderMetadata, error)
}

// 新增：LibraryUsecaseWithScan 接口，包含扫描相关方法
type LibraryUsecaseWithScan interface {
	LibraryUsecase
	SetFileUsecase(uc *fileUsecaseCore.FileUsecase)
}

type libraryUsecase struct {
	folderRepo  folderCore.FolderRepository
	fileUsecase *fileUsecaseCore.FileUsecase
}

func NewLibraryUsecase(repo folderCore.FolderRepository) LibraryUsecaseWithScan {
	return &libraryUsecase{folderRepo: repo}
}

// 新增：SetFileUsecase 设置FileUsecase引用
func (uc *libraryUsecase) SetFileUsecase(fileUsecase *fileUsecaseCore.FileUsecase) {
	uc.fileUsecase = fileUsecase
}

func (uc *libraryUsecase) BrowseFolders(ctx context.Context, path string) ([]folderCore.FolderEntry, error) {
	return uc.folderRepo.ListFolders(path)
}

func (uc *libraryUsecase) CreateLibrary(ctx context.Context, name, path string, folderType int) (*folderCore.LibraryFolderMetadata, error) {
	// 路径验证
	...
	if uc.fileUsecase != nil {
		...
		if activeScan {
			return nil, folderCore.ErrLibraryInUse
		}
	}

	library := &folderCore.LibraryFolderMetadata{
		Name:       name,
...
func (uc *libraryUsecase) DeleteLibrary(ctx context.Context, id string) error {
	...
	if lib, err := uc.folderRepo.GetByID(ctx, objID); err == nil && lib.Status == folderCore.StatusScanning {
...
func (uc *libraryUsecase) GetLibraries(ctx context.Context) ([]*folderCore.LibraryFolderMetadata, error) {
