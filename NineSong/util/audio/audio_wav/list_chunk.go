...
var (
	...
	markerLYR     = [4]byte{'L', 'Y', 'R', ' '} // 歌词标记 (LRC)
	markerAPIC    = [4]byte{'A', 'P', 'I', 'C'} // ID3v2 APIC 封面图像标记
	markerCOVR    = [4]byte{'C', 'O', 'V', 'R'} // 备用封面图像标记
)
...
func DecodeListChunk(d *Decoder, ch *riff.Chunk) error {
	...
	if ch.ID == CIDList {
		...
		for err == nil {
			...
			case markerLYR:
				d.Metadata.Lyrics = nullTermStr(scratch)
			case markerAPIC, markerCOVR:
				if size > 0 {
					d.Metadata.CoverImage = make([]byte, size)
					copy(d.Metadata.CoverImage, scratch)
				}
			}
			if size%2 == 1 {
