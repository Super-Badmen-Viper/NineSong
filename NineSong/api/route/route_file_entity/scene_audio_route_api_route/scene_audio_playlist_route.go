...
func NewPlaylistRouter(
	...
	repo := scene_audio_route_repository.NewPlaylistRepository(db, domain.CollectionFileEntityAudioScenePlaylist)
	tempRepo := scene_audio_db_repository.NewTempRepository(db, domain.CollectionFileEntityAudioSceneTempMetadata)
	usecase := scene_audio_route_usecase.NewPlaylistUsecase(repo, tempRepo, timeout)
